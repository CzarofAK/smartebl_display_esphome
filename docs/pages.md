...
- Contents: exactly the 5 switches from `docs/protocol.md` group 4 that
  ...
  first.
- Alarmo is deliberately **not** in this popup — see §6. (Fridge was also excluded here; as of 2026-09-26 there is no fridge switch at all, see §6.)
...
|---|---|---|---|---|
| 0 | Home | — | Digital clock (right half, §6) + 5 switch tiles + Alarmo tile (§6). The Fridge tile is commented out since 2026-09-26, see §6 | link (switches) + HA (clock temp) |
...
| — | *Fresh/Waste source, TEMPORARY (2026-09-12)* | — | Per explicit repo-owner request: Fresh/Waste now read from Victron via HA (`sensor.fresh_stand`/`sensor.waste_stand`) instead of `smartebl`'s own link (`tank1_level`/`tank2_level`) - called out by the repo owner as a temporary fix, not a decision to drop the link source for good. `s_tank1_level`/`s_tank2_level` and their two cases in the link reader are commented out in the YAML rather than deleted, specifically so reverting is a small, mechanical uncomment rather than re-deriving anything (see `smart_ebl_pages/page_levels.yaml`'s own sensor: block comment). The whole page is HA-fed for now, so the "no data at all" fallback line was renamed from "KEINE VERBINDUNG" (specifically about the link) to "KEINE DATEN". | HA (all five, for now) |
...
| 7 | Sensors | 1: Levelling, 2: Fenster/Türen, 3: Klappen | Page 1: spirit level + per-corner cm-to-add, same 4 entities `m5dial_fram/page_levelling.yaml` reads; page 2: 11 door/window contacts; page 3: 7 flap/hatch contacts (both grid layouts per `smartebl_display/docs/design.md`'s "Status Grid Overview" template, extended with battery/voltage/temperature/last-seen) | HA |
| — | *implemented, page 3 only (`page_sensors_levelling`)* | — | **Levelling built**, ported from `m5dial_fram/page_levelling.yaml` verbatim (same bubble-target-rings metaphor, same worst-corner-wins bubble color, same still-open item there: the four corner cm sensors read 0 until an HA-side template sensor does the real degrees-to-cm math - nothing to change here once that lands). Read-only, no button, matches that file. **Pages 1/2 were an honest placeholder** (`page_sensors_overview`, a single 6-cell mockup, no real entities) until the SENSORS nav entry + its own "1/2" indicator (§2's pattern) made Levelling reachable without waiting on it - see the row below for the real build that replaced it. | HA |
| — | *implemented, pages 1+2, real entity list (2026-09-19)* | — | **Placeholder replaced.** Repo owner supplied the real 18-entity door/window/flap list, split into two sub-pages rather than one (`page_sensors_windows.yaml`, 11 contacts: Beifahrer-/Fahrer-/Eingangstür, Küchen-/Tisch-/beide Bettfenster, beide Miniheki, Midiheki, Heki 3; `page_sensors_flaps.yaml`, 7 contacts: Gasflaschen-, beide Garagenklappen, Service-, Schuh-, Katzenklo-, Wäscheklappe) - Sensors is now a 3-way "1/3"/"2/3"/"3/3" cycle (`nav_show_sensors`/`draw_nav`, main device file), Levelling moved from sub-page 2 to sub-page 3. Every contact is Home Assistant-fed (Zigbee-style per-device diagnostics, same naming convention for all 18): `binary_sensor.<name>_contact` (device_class door/window ASSUMED - "on"=OFFEN/orange, "off"=GESCHLOSSEN/green, not separately confirmed per-entity) + `sensor.<name>_battery`/`_voltage`/`_device_temperature`, each rendered `---`/greyed independently per design_rules.md §2's invalid-value rule, not as one combined per-card flag. `sensor.<name>_last_seen` (an HA timestamp string) is deliberately **not** timezone-parsed - no confirmed local-timezone conversion exists anywhere else in this codebase to reuse, and a silently-wrong absolute time would be worse than not showing one; instead each card tracks, in `millis()`, when this display last observed that entity change, and shows elapsed time since ("vor Xm"/"vor Xh Ym") - a good staleness proxy since these entities only change when the physical sensor actually reports, at the stated cost that every card reads "vor <1 Min" right after this display reboots regardless of the sensor's real last report. See `page_sensors_windows.yaml`'s own header for the full grid-math/font-glyph reasoning (kept out of this table). | HA |
| — | *reordered, round 2 (2026-09-19)* | — | Repo-owner: "3 (Libelle und Hubstuetzen) soll auf Pos. 1 und die anderen 2 und 3" - Levelling moved from sub-page 3 to sub-page 1 (front of the cycle), Fenster/Türen and Klappen keep their own relative order, shifted down to sub-pages 2/3. Only the order in `nav_show_sensors`'s if/else chain changed (main device file) - none of the three page files themselves were touched. | — |
| — | *relayout, page 2 (2026-09-06)* | — | Per repo-owner spec ("das Widget ist VIEL zu klein! eine Seite der freien Flaeche benutzen. Die andere Haelfte ist fuer die Huebstuetzen-Kontrolle reserviert"): split at x=705 (same left/right convention as the Fans page's own divider). **Left half:** the same bubble-level widget, everything doubled (200x200 root -> 400x400, every ring/bubble/offset x2 with it) so it actually fills its half instead of floating small in the middle of the whole content area. **Right half:** Huebstuetzen (leveling jacks) - repo-owner-confirmed the jacks aren't physically installed in this motorhome yet, so this is a genuinely **reserved placeholder** (permanently invalid/grey style, no live entity, no on_click - the treatment the Home page's Fridge tile used to get too, before that one was commented out on 2026-09-26 for a fridge switch that will never exist; the jacks are still genuinely planned, so this one stays reserved) rather than a panel wired against guessed entity_ids that would just log HA warnings for nothing. Laid out for the control the repo owner described for once the hardware exists - one ON/OFF switch, one AUTOLEVEL button, a status indicator (green = eingefahren/retracted, red = ausgefahren/extended) - "maybe others", so not assumed final. An earlier round of this file built a fully-wired per-corner AUSFAHREN/EINFAHREN panel against placeholder entity_ids including a retract-confirm popup (`m5dial_fram/design_rules.md` §14's long-open "Confirmation for dangerous buttons" item) - reverted once the repo owner confirmed the hardware doesn't exist; that confirm-popup pattern is still the right one to reuse once real entities land. | HA |
...
The middle-right slot held the reserved Fridge tile until 2026-09-26;
see the table's Fridge row for why it is now empty.
...
| Fridge | none — **resolved NO, 2026-09-26** | This section's "reserve the slot regardless of outcome" rule is **closed**: the repo owner confirmed this vehicle has no fridge switch ("Fridge switch habe ich nicht, bitte auskommentieren"), so the tile is **commented out** in `page_home.yaml` (not deleted — `smartebl` still supports a fridge line, F9/F10, `docs/protocol.md`'s planned `fridge_control`, so another vehicle or a rewire could want it back; the block keeps its indentation so re-enabling it is a pure uncomment). The middle-right slot is simply empty. The original worry behind reserving it — "the layout would shift later" — does not apply: every tile has absolute x/y, so removing one moves nothing |
...
- ~~**Sensors page 1 content.**~~ Resolved, 2026-09-19 - real 18-entity
  door/window/flap list supplied by the repo owner, split across two
  real sub-pages (`page_sensors_windows.yaml`, `page_sensors_flaps.yaml`)
  per §5's own table row above; the single 6-cell placeholder grid
  (`page_sensors_overview.yaml`) is gone.
