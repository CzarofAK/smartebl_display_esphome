...
| 7 | Sensors | 1: Fenster/Türen, 2: Klappen, 3: Levelling | Page 1: 11 door/window contacts; page 2: 7 flap/hatch contacts (both grid layouts per `smartebl_display/docs/design.md`'s "Status Grid Overview" template, extended with battery/voltage/temperature/last-seen); page 3: spirit level + per-corner cm-to-add, same 4 entities `m5dial_fram/page_levelling.yaml` reads | HA |
